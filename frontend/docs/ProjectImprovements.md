...
Fixed a major bug in the application where applicants can only upload one resume for every job. Now, applicants are allowed to upload a unique resume for each job listing. Also another bug was that whenever the applicant changed their resume in their profile, the resume was automatically changed in the managers portal. Now the resume in the managers portal is application specific.
